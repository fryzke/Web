...
<div>
		<table class="table">
			...
			<tbody>
				<%
					board_listing bd_list = new board_listing();
					ArrayList<board_db> list = bd_list.getList(pageNum);
					for(int i=0; i<list.size(); i++){
						String st;
						if(list.get(i).getAns_st() == 1){
							st = "답변완료";
						}else{
							st = "답변대기";
						}
				%>
					<tr>
						...
						<td><a href="view.jsp?logid=<%= list.get(i).getLogid() %>">
							<%= list.get(i).getLog_title() %></a></td>
						<td><%= list.get(i).getWriter() %></td>
						<td><%= list.get(i).getLog_date().substring(0, 10) %></td>
						<td><%= st %></td>
				</tr>
				<%
					}
		...
		<%
			}if(bd_list.nextPage(pageNum+1)){
		%>
			<a href="board.jsp?pageNum=<%= pageNum + 1 %>" class="btn btn-success btn-arrow-left">다음</a>
		<%
			}
		%>
		<hr>
<div><a href="write_log.jsp" class="btn btn-primary pull-right">글쓰기</a></div>
</div>
